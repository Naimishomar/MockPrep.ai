...
exports.userLogin = async(req,res)=>{
    try {
        ...
        if(!username || !password){
            ...
            const token = jwt.sign({ userId: user._id }, process.env.JWT_SECRET,{ expiresIn: '5d' });
            res.cookie('token', token, {httpOnly: true,sameSite: 'strict', maxAge: 5 * 24 * 60 * 60 * 1000});
            const userDetails = {
                _id: user._id,
                name: user.name,
            ...
            return res.status(200).json({message: `Logged in successfully, hello ${user.name}`, success: true, token, userDetails});
        }
    } catch (error) {
        console.log(error);  
    }
...
exports.getProfile = async (req, res) => {
  try {
    ...
    const user = await User.findById(decoded.userId).select("-password").populate("history", "name username email") // Populate history (if used as linked users) 
    if (!user) {
...
exports.history = async(req,res)=>{
  try {
    ...
    if(!title){
      res.status(500).json({message:"No search details found", success: false});
    }
    else{
      ...
      const updatedUser = await User.updateOne({$push:{ history: history._id}});
      await updatedUser.populate('history');
      res.status(200).json({message:"History has been updated", success: true, userWithHistory});
    }
  } catch (error) {
    console.log(error);
  }
